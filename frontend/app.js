...
/* document.addEventListener('DOMContentLoaded', () => {
  ...
  const renderFilas = (items) => {
    ...
    items.forEach(doc => {
      ...
      tr.innerHTML = `
        ...
        <td class="text-center">${publicaciones}</td>
        <td class="text-center">
          <button class="btn btn-sm btn-outline-primary btn-ver" data-id="${doc._id}">
            <i class="bi bi-eye"></i>
          </button>
          <button class="btn btn-sm btn-warning btn-editar" data-id="${doc._id}">
            <i class="bi bi-pencil-square"></i>
          </button>
          <button class="btn btn-sm btn-danger btn-eliminar" data-id="${doc._id}">
            <i class="bi bi-trash"></i>
          </button>
        </td>
      `;
  ...
  const listar = async () => {
    try {
      ...
      const q = txtBuscar.value?.trim().toLowerCase();
      if (q) {
        data = data.filter(r => [r.nom_fan_pag, r.des_fan_pag, r.per_fan_pag]
          .some(v => String(v ?? '').toLowerCase().includes(q)));
      }
      renderFilas(data);
    } catch (err) {
  ...
  tablaBody.addEventListener('click', async (e) => {
    ...
    if (e.target.closest('.btn-ver')) {
      ...
      detalleBody.innerHTML = `
        <h5>${data.nom_fan_pag}</h5>
        <p>${data.des_fan_pag}</p>
        <p><strong>Avatar:</strong> ${data.per_fan_pag}</p>
        <p><strong>Fecha:</strong> ${fmtFecha(data.fec_fan_pag)} | ${badgeEstado(data.est_fan_pag)}</p>
        <hr>
        <h6>Categorías</h6>
        <ul>${data.categoria?.map(c=>`<li>${c.nom_cat} - ${c.des_cat}</li>`).join('')||'Ninguna'}</ul>
        <h6>Publicaciones</h6>
        ${data.publicacion?.map(p=>`
          <div class="mb-3">
            <strong>${p.tit_pub}</strong><br>
            <small>${fmtFecha(p.fec_pub)}</small><br>
            <p>${p.des_pub}</p>
            ${p.multimedia?.map(m=> m.tip_mul==='imagen'
              ? `<img src="${m.url_mul}" class="img-fluid rounded mb-2" style="max-height:150px;">`
              : `<video src="${m.url_mul}" class="w-100 mb-2" controls></video>`
            ).join('') || ''}
          </div>`).join('')||'Ninguna'}
      `;
      modalDetalle.show();
    ...
    if (e.target.closest('.btn-eliminar')) {
      if (confirm('¿Eliminar esta fan page?')) { await eliminar(id); listar(); }
    }
  });
